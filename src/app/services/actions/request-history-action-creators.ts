...
import {
  ...
  removeHistoryData,
} from '../../views/sidebar/history/history-utils';
import {
...
export function removeHistoryItem(historyItem: IHistoryItem): Function {
  ...
  return async (dispatch: Function) => {
    return removeHistoryData(historyItem).then(() => {
      dispatch({
...
export function bulkRemoveHistoryItems(
  historyItems: IHistoryItem[] | null,
  all?: boolean
): Function {
  ...
  return async (dispatch: Function) => {
    return bulkRemoveHistoryData(listOfKeys, all).then(() => {
      dispatch({
